...
public class CalController {
	...
	//캘린더 검색 리스트
	@RequestMapping(value = "/calSearch.do", method = RequestMethod.GET)
	public String CalSearch(HttpServletRequest request, Locale locale, Model model) {
		logger.info("캘린더보기", locale);	
		List<CalDto> selectList = new ArrayList<CalDto>();
		List<CalDto> calViewList = calService.getCalList();
		String search = request.getParameter("academy_search");
	...
	public String CalDetail(HttpServletRequest request, Locale locale, Model model, String ac_seq) throws UnsupportedEncodingException {
		request.setCharacterEncoding("utf-8");
		logger.info("캘린더상세보기", locale);
		String ac_sseq=request.getParameter("ac_seq");
		int ac_seqq=Integer.parseInt(ac_sseq);
		model.addAttribute("calDto", calService.getCalDetail(ac_seqq));
	...
	//장바구니 페이지
		@RequestMapping(value = "/cartAcademyCal.do", method = RequestMethod.GET)
			public String cartAcademyCal(HttpServletRequest request, Locale locale, Model model) throws UnsupportedEncodingException {
				logger.info("캘린더장바구니", locale);
